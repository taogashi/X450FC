...
void SD740_SPI_Init(void)
{
	...
	RCC_APB2PeriphClockCmd(SD740_SPI_RCC_Periph,ENABLE);
	RCC_AHB1PeriphClockCmd(SD740_SPI_RCC_Port,ENABLE);

	GPIO_PinAFConfig(SD740_SPI_NSS_Pin_Port, SD740_SPI_NSS_Pin_Source, SD740_SPI_NSS_AF);
	GPIO_PinAFConfig(SD740_SPI_CLK_Pin_Port, SD740_SPI_CLK_Pin_Source, SD740_SPI_CLK_AF);
	GPIO_PinAFConfig(SD740_SPI_MISO_Pin_Port, SD740_SPI_MISO_Pin_Source, SD740_SPI_MISO_AF);
	GPIO_PinAFConfig(SD740_SPI_MOSI_Pin_Port, SD740_SPI_MOSI_Pin_Source, SD740_SPI_MOSI_AF);

	GPIO_InitStructure.GPIO_Pin =SD740_SPI_CLK_Pin | SD740_SPI_MISO_Pin | SD740_SPI_MOSI_Pin;
...
void SD740_Read_GyroRate(float* out)
{
	...
	for(i=0;i<3;i++)
	{	
		rawData[i]-=SD740_offset[i];
		out[i]=rawData[i]*0.03125;
	}

}

void SD740_Cali(void)
{
	u32 k;
	u16 i;
	u8 j;
	...
	for(j=0;j<3;j++)
		SD740_offset[j]=(s16)(sum[j]/300.0);	
}
