...
void USART2_DMA_Config(void)
{
	...
	uart2_user_tx_buffer.header = 0;
	uart2_user_tx_buffer.tail = 0;
}
...
u8 Uart2Send(char* content, u16 num)
{
	...
	occupied_buffer = uart2_user_tx_buffer.header - uart2_user_tx_buffer.tail;
	...
	if(DMA_Transmit_Complete == 1)
	{
		...
		if(total_byte <= UART_TX_BUFFER_LENGTH)
		{
			...
			{
				uart_dma_buffer[i] = uart2_user_tx_buffer.buffer[uart2_user_tx_buffer.tail++];
				if(uart2_user_tx_buffer.tail >= UART_TX_BUFFER_LENGTH)
					uart2_user_tx_buffer.tail = 0;
			}
			for(i=0; i<num; i++)
			{
		...
		else
		{
			...
			{
				uart2_user_tx_buffer.buffer[uart2_user_tx_buffer.header++] = content[UART_TX_BUFFER_LENGTH-occupied_buffer+i];
			}
	...
	else
	{
		...
		else
		{
			for(i=0; i<num; i++)
			{
				uart2_user_tx_buffer.buffer[uart2_user_tx_buffer.header++] = content[i];
				if(uart2_user_tx_buffer.header >= UART_TX_BUFFER_LENGTH)
					uart2_user_tx_buffer.header = 0;
			}
			return 1;
		}
