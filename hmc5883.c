...
void HMC5883_I2C_Init(void)	//初始化I2C总线
{
  ...
  RCC_AHB1PeriphClockCmd(HMC5883_I2C_RCC_Port, ENABLE);
  ...
  GPIO_Init(HMC5883_I2C_Port, &GPIO_InitStructure);
	...
	GPIO_PinAFConfig(HMC5883_I2C_Port, HMC5883_I2C_SCL_Pin_Source, GPIO_AF_I2C2);
	GPIO_PinAFConfig(HMC5883_I2C_Port, HMC5883_I2C_SDA_Pin_Source, GPIO_AF_I2C2);
...
u8 HMC5883_Read_Raw(s16* mag)//读取磁罗盘数据 
{
	u8 i;
	u8 buffer[6];
	u8 status;
...
void HMC5883_Read_mag(s16* mag)//读取磁罗盘数据 
{
	s16 rawData[3];

	if(1==HMC5883_Read_Raw(rawData))
	{
//		mag[0]=-(s16)(0.8275*(rawData[0]+11))-3;
//		mag[1]=(s16)(0.8127*(rawData[2]+155))+8;
//		mag[2]=-(s16)(0.956*(rawData[1]-125))-1;

		mag[0]=-(s16)(0.8127*(rawData[2]+165));
		mag[1]=-(s16)(0.8275*(rawData[0]+15));
		mag[2]=-(s16)(0.956*(rawData[1]-124));
	}
}
